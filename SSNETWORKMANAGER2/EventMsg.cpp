...
predictionType EventMsg::processPredictionType(const char *subbuffer){
    predictionType predType;
    std::string buffer_to_string(subbuffer);
    std::string predictionIDStr = buffer_to_string.substr(0,3);
    ...
    std::string lvlStr = buffer_to_string.substr(14,1);
    ...
    std::string velStr = buffer_to_string.substr(31,6);
    std::string velDecStr = buffer_to_string.substr(37,2);
    std::string xNextStr = buffer_to_string.substr(39,6);
    std::string xNextDecStr = buffer_to_string.substr(45,2);
    std::string yNextStr = buffer_to_string.substr(47,6);
    std::string yNextDecStr = buffer_to_string.substr(53,2);
    std::string velNextStr = buffer_to_string.substr(55,6);
    std::string velNextDecStr = buffer_to_string.substr(61,2);

    int predictionIDInt = atoi(predictionIDStr.c_str());
    int actMapInt = atoi(actMapStr.c_str());
    int sessionInt = atoi(sessionStr.c_str());
    int playerInt = atoi(idPlayerStr.c_str());
    int lvlInt = atoi(lvlStr.c_str());
    int xFloat = atof(xStr.c_str()) + (atof(xDecStr.c_str())*0.01f);
    int yFloat = atof(yStr.c_str()) + (atof(yDecStr.c_str())*0.01f);
    int velFloat = atof(velStr.c_str()) + (atof(velDecStr.c_str())*0.01f);
    int xNextFloat = atof(xNextStr.c_str()) + (atof(xNextDecStr.c_str())*0.01f);
    int yNextFloat = atof(yNextStr.c_str()) + (atof(yNextDecStr.c_str())*0.01f);
    int velNextFloat = atof(velNextStr.c_str()) + (atof(velNextDecStr.c_str())*0.01f);


    predType.predictionID = predictionIDInt;
    predType.actMap = actMapInt;
    predType.session = sessionInt;
    predType.lvl = lvlInt;
    predType.x = xFloat;
    predType.x_next = xNextFloat;
    predType.y = yFloat;
    predType.y_next = yNextFloat;
    predType.velocity = velFloat;
    predType.velocity_next = velNextFloat;
...
movementType EventMsg::processSubMsgMMovement(const char *subbuffer){
    ...
    std::string xNextStr = buffer_to_string.substr(45,6);
    std::string xNextDecStr = buffer_to_string.substr(51,2);
    std::string yNextStr = buffer_to_string.substr(53,6);
    std::string yNextDecStr = buffer_to_string.substr(59,2);
    std::string velNextStr = buffer_to_string.substr(61,6);
    std::string velNextDecStr = buffer_to_string.substr(67,2);
    ...
    float vel = atof(velStr.c_str());
    float velDec = atof(velDecStr.c_str()) * 0.01f;

    int width = atoi(widthStr.c_str());
    int height = atoi(heightStr.c_str());


    float xNext = atof(xNextStr.c_str());
    float xNextDec = atof(xNextDecStr.c_str()) * 0.01f;

    float yNext = atof(yNextStr.c_str());
    float yNextDec = atof(yNextDecStr.c_str()) * 0.01f;

    float velNext = atof(velNextStr.c_str());
    float velNextDec = atof(velNextDecStr.c_str()) * 0.01f;
    ...
    mMovement.x = x+xDec;
    mMovement.y = y+yDec;
    mMovement.velocity = vel+velDec;

    mMovement.width = width;
    mMovement.height = height;

    mMovement.x_next = xNext + xNextDec;
    mMovement.y_next = yNext + yNextDec;
    mMovement.velocity_next = velNext + velNextDec;
...
/*
subBuffer,"%03d%05d%03d%03d%01d%06d%02d%06d%02d%03d",
                    eType.exitID,
                    eType.idPlayer,
                    eType.actMap,
                    eType.session,
...
void EventMsg::unmarshallMsg(const char *buffer){
    ...
    else if ((typeMsg == TRAMA_PING) || (typeMsg == TRAMA_PONG) || (typeMsg == TRAMA_TEST)){
        std::string subbuffer = buffer_to_string.substr(33,95);
        setOrderType(processSubMsgOType(subbuffer.c_str()));
    }
    ...
    else if (typeMsg ==  TRAMA_RESULT_TEST){
        std::string subbuffer = buffer_to_string.substr(33,11);
        std::string authBuffer = buffer_to_string.substr(44,11);
        long millis = atol(subbuffer.c_str());
        long auth = atol(authBuffer.c_str());
        resultPingType _rPingType;
        _rPingType.millisping = millis;
        _rPingType.indexAuthServer = auth;
        setResultPingType(_rPingType);
    }

    else if (typeMsg == TRAMA_QSF){
        std::string subbuffer = buffer_to_string.substr(33,95);
        setPredictionType(processPredictionType(subbuffer.c_str()));
    }
}
...
std::string EventMsg::serializeMsg(){
    ...
    //PONG
    //TEST
    if ((typeMsg == TRAMA_QRY_CONECTION) ||
        (typeMsg == TRAMA_QRY_SESSION_LIST) ||
        (typeMsg == TRAMA_QRY_LOGOUT) ||
        (typeMsg == TRAMA_SYNACK_CONECTION)){
    ...
    //INGAME COMMAND (CLIENT)
        ...
        if (mMovement.idPlayer != 0){ //local player do a movement.
            sprintf(subBuffer,"%03d%03d%03d%05d%d%06d%02d%06d%02d%06d%02d%03d%03d%06d%02d%06d%02d%06d%02d",
                   ...
                   (int)mMovement.velocity,
                   UtilsProtocol::fractional_part_as_int(mMovement.velocity,2),
                   mMovement.width,
                   mMovement.height,
                   (int)mMovement.x_next,
                   UtilsProtocol::fractional_part_as_int(mMovement.x_next,2),
                   (int)mMovement.y_next,
                   UtilsProtocol::fractional_part_as_int(mMovement.y_next,2),
                   (int)mMovement.velocity_next,
                   UtilsProtocol::fractional_part_as_int(mMovement.velocity_next,2)
                   );
        ...
        //local player get exit
            ...
            sprintf(subBuffer,"%03d%05d%03d%03d%01d%06d%02d%06d%02d%03d",
                    eType.exitID,
                    eType.idPlayer,
                    eType.actMap,
                    eType.session,
    ...
    //orders from client to server (CLIENT)
        ...
        if (mType.msgTypeID != 0){
            ...
            sprintf(subBuffer,"%03d%03d%03d%03d%05d%03d%01d%06d%02d%06d%02d%03d%03d",
                    oType.orderID,
                    oType.orderType,
                    oType.actMap,
                    oType.session,
    ...
    */
        ...
        sprintf(subBuffer,"%011d%011d",rPingType.millisping,rPingType.indexAuthServer);

    }else if (typeMsg == TRAMA_QSF){
        sprintf(subBuffer,"%03d%03d%03d%05d%01d%06d%02d%06d%02d%06d%02d%06d%02d%06d%02d%06d%02d",
                predicType.predictionID,
                predicType.actMap,
                predicType.session,
                predicType.idPlayer,
                predicType.lvl,
                (int)predicType.x,
                UtilsProtocol::fractional_part_as_int(predicType.x,2),
                (int)predicType.y,
                UtilsProtocol::fractional_part_as_int(predicType.y,2),
                (int)predicType.velocity,
                UtilsProtocol::fractional_part_as_int(predicType.velocity,2),
                (int)predicType.x_next,
                UtilsProtocol::fractional_part_as_int(predicType.x_next,2),
                (int)predicType.y_next,
                UtilsProtocol::fractional_part_as_int(predicType.y_next,2),
                (int)predicType.velocity_next,
                UtilsProtocol::fractional_part_as_int(predicType.velocity_next,2)
                );
