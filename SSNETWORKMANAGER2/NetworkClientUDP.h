...
EventMsg *getMsgFromServerQueue();

EventMsg **getMsgsFromServer();

void initCommunicationUDP(std::string _ip, int _remotePort, int _localPort);
void initCommunicationUDP(std::string _ip, int _remotePort, int _localPort, int _remotePort_1, int _localPort_1);

void establishCommunicationUDP();
void getListActiveSessions();
void registerToActiveSession(int mapClient, int sessionClient);

resultPingType sendPingAndResult(int mapClient, int sessionClient, int playerID);

private:

IPaddress serverIP;
IPaddress serverIP_1;

UDPsocket clientSocket;
UDPsocket clientSocket_1;
