...
NetworkClientUDP *nClientUDPQueue;
...
void createEnvirontmentNew(){
    ...
    //nClientUDPQueue = new NetworkClientUDP(logger);

    std::cout << "\r\nEnter local port SSNETWORKMANAGERSERVER: ";
    std::cin >> port;

    //udp port for broadcasting info world information
    portQueue = port + 1;
    logger->info("[SSNETWORKMANAGERSERVER::Main] OPEN LOCAL PORT (%d) QUEUE PORT (%d)", port, portQueue);

    nClientUDP->initCommunicationUDP(port,portQueue);
    //nClientUDPQueue->initCommunicationUDP(portQueue);
