...
void DynamicEntity::VOnUpdate(long deltaMs){
    ...
    logPackets->debug("[DYNAMICENTITY:SETDYNAMICENTITY:CLIENT] SERVER_STEP[%d] HOME POS ENTITY ID:[%d] (ActMap:%d,Session:%d,lvl:%d,x:%f,y:%f)",
                        deltaMs, this->idDE,this->actMap,this->actSession,this->actLevel,this->x,this->y);
}
