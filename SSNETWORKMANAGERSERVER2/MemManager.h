...
#define WORLD_PHOTO_SIZE 20

#include "WorldPhoto.h"


typedef std::pair<long, WorldPhoto *> WorldType;

struct CompareSecond
{
    bool operator()(const WorldType& left, const WorldType& right) const
    {
...
MemManager(){
    ...
    timeServerM = SDL_CreateMutex();

    timeStampServerM = SDL_CreateMutex();
...
long getSizeTimeServer(){
    long data = 0;
    SDL_LockMutex(timeServerM);
    data = sizeTimeServer;
    SDL_UnlockMutex(timeServerM);
    return data;
}

void setSizeTimeServer(long _sizeTimeServer){
    SDL_LockMutex(timeServerM);
    this->sizeTimeServer = _sizeTimeServer;
    SDL_UnlockMutex(timeServerM);
}

long getTimeStampServer(){
    long data = 0;
    SDL_LockMutex(timeStampServerM);
    data = timeStampServer;
    SDL_UnlockMutex(timeStampServerM);
    return data;
}

void setTimeStampServer(long _timeStampServer){
    SDL_LockMutex(timeStampServerM);
    this->timeStampServer = _timeStampServer;
    SDL_UnlockMutex(timeStampServerM);
}

void addWorldPhotoMap(long key, WorldPhoto *data){
    std::pair<long, WorldPhoto *> node(key, data);
    worldPhotoMap.insert(node);
};


std::map<long, WorldPhoto *>getPhotoMap(){return worldPhotoMap;}

long getMinWorldIndex()
{
  std::pair<long, WorldPhoto *> min = *min_element(worldPhotoMap.begin(), worldPhotoMap.end(), CompareSecond());
  return min.first;
}

void removeOldWorldPhoto()
{
    std::map<long, WorldPhoto *>::iterator it;
    long worldLowIndex = getMinWorldIndex();
    it=worldPhotoMap.find(worldLowIndex);
    worldPhotoMap.erase (it);
}
...
std::map<long, WorldPhoto *>worldPhotoMap;
...
SDL_mutex *timeServerM;
SDL_mutex *timeStampServerM;
/********************************************************/
