...
void NetworkClientUDP::initCommunicationUDP(Uint16 localPort_1, Uint16 localPort_2){
    ...
    serverSocket = SDLNet_UDP_Open(localPort_1);

    if (serverSocket==NULL){
        logger->error("[SSNETWORKMANAGERSERVER::initCommunicationUDP] Error creating serverSocket [%s]",SDLNet_GetError());
        exit(-1);
    }else{
        logger->debug("[SSNETWORKMANAGERSERVER::initCommunicationUDP] listen from 0.0.0.0/[%d]",localPort_1);
    }

     serverSocket_1 = SDLNet_UDP_Open(localPort_2);

    if (serverSocket_1==NULL){
        logger->error("[SSNETWORKMANAGERSERVER::initCommunicationUDP] Error creating serverSocket [%s]",SDLNet_GetError());
        exit(-1);
    }else{
        logger->debug("[SSNETWORKMANAGERSERVER::initCommunicationUDP] listen from 0.0.0.0/[%d]",localPort_2);
    }
...
//-->SEND ONE MSG TO CLIENT QUEUE(S)
void NetworkClientUDP::sendMsgToClientUDPQueue(EventMsg *msg){
    ...
    if ( SDLNet_UDP_Send(serverSocket_1, -1, packetUDP) == 0 ){
...
EventMsg *NetworkClientUDP::getMsgFromClientUDP(){
    ...
    while(!DONE){
		...
		if (!DONE){
            ...
            if (DIFF >= MAX_DELAY){
                logger->warn("[SSNETWORKMANAGERSERVER::getMsgFromClientUDP] --> SOCKET INACTIVE DURING [%d] MS SYSTEMOUT", DIFF);
                DONE = true;
            }
            SDL_Delay(10);
		}
