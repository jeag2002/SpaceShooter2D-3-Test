...
void DBConnection::createConn(){

   if ( sqlite3_open(DB, &dbfile) == SQLITE_OK ){
        logger->info("[DBConnection::createConn] connection successfully to local database [%s]",DB);
   }else{
        logger->warn("[DBConnection::createConn] connection error to local database [%s]",DB);
   }
