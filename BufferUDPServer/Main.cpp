...
int main(int argc, char **argv){
	...
	UDPpacket **bufferpackets;
    UDPpacket **out;

	bool endTransmision = false;

	out = SDLNet_AllocPacketV(12, 100);
    ...
    while(!endTransmision){
        ...
        for(int i=0; i<30; i++){packet->data[i]=(Uint8)'\0';}

        if ( SDLNet_UDP_Recv(ourSocket, packet))
		{
			std::cout << "\tData received from CLIENT: " << packet->data << "\n";

			fprintf(pFile,"[GET CLIENT MSG FROM CLIENT] Data received from CLIENT: %s\n", packet->data);
            ...
            if(host){
               ...
               fprintf(pFile,"[GET CLIENT MSG FROM CLIENT] request from host: %s port: %d\n", host, port);
            }else{
			...
			num_trama++;
            for(int index_packet = 0; index_packet < 12; index_packet++){
                for(int i=0; i<100; i++){buffer[i]='\0';}
                sprintf(buffer,"(#)MSG TO CLIENT TRAMA_SERVER (%d) - packet %d", num_trama, index_packet);
                std::string msg(buffer);

                UDPpacket *packet_data = SDLNet_AllocPacket(100);

                for(int i=0; i<100; i++){packet_data->data[i]='\0';}

                memcpy(packet_data->data, msg.c_str(), msg.length() );

                packet_data->len = msg.length();
                packet_data->channel = packet->channel;
                packet_data->status = packet->status;
                packet_data->address.host = packet->address.host;
                packet_data->address.port = packet->address.port;

                std::cout << "\tData send to CLIENT: " << packet_data->data << " len " << packet_data->len <<"\n";
                fprintf(pFile,"[SEND SERVER MSG TO CLIENT][%d] [%s] size [%d]\n", index_packet, packet_data->data, packet_data->len);
                out[index_packet] = packet_data;
            }

/**<  */
            if(SDLNet_UDP_SendV(ourSocket,out, 12)<12){
                std::cout << "SDLNet_UDP_SendV: " << SDLNet_GetError() << "\n";
                exit(-1);
            }

             std::cout << "SDLNet_UDP_SendV: Buffer SEND\n";
             fprintf(pFile,"[SERVER] SDLNet_UDP_SendV: Buffer SEND\n");
